...
public class Demand extends JInternalFrame implements ActionListener,FocusListener
 { 
	...
	JComboBox d1,d2,d3,m1,m2,m3,y1,y2,y3,area_code,day_code,mtr_code,con_no;
	JButton qry,bd,mrd,exit,edit,ok;
	...
	public 	Demand(String title)
  	{
		...
		mtr_code=new JComboBox(mc);
		area_code.addActionListener(this);
		mtr_code.addActionListener(this);
		
		con_no=new JComboBox(v2);
...
cld=new JTextField(10);
		...
		qry=new JButton("Querry");
		
		FlowLayout layout=new FlowLayout();
		bd=new JButton("Show Bill details");
 ...
 //----------------------------------------------------}
	...
	//___________________________________________________________________________
		...
		if((e.getSource()==area_code)||(e.getSource()==mtr_code))
		{
			...
			v2.clear();
			v2.addAll(qd.getCons((String)area_code.getSelectedItem(),(String)mtr_code.getSelectedItem()));
	...
	if((e.getSource()==area_code)||(e.getSource()==mtr_code)||(e.getSource()==con_no))
		{
			System.out.println("Selected con_no,area_code,mtr_code");
				
			 try{
			QueryData qd = new QueryData();
		
						String id=(String)con_no.getSelectedItem();//||(String)mtr_code.getSelectedItem()||(String)con_no.getSelectedItem());
	...
	//====================================================					
				...
				try{
					...
					pst2.setString(3,(String)con_no.getSelectedItem());
